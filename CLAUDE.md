...
**Current state (2026-07-04):** data acquired; A1 spec sheet locked; **v1 derating curve done + calibration-tuned (v1.1)** — open-air Brayton, −17.7% power at 55 °C, ≈0.60 %/°C ≈ 4× Carnot; calibration fixes η_mech=0.93 and solves TIT=742 °C (in Na heat-pipe band). Next: CFD/v2 (re-scoped to the open-air Brayton hot-side HX + intake; start with the low-effort ε-NTU model), then wire v1 curve into the Phase-2 siting pipeline. Nothing committed to git yet — if initializing, gitignore `data/nuclear_benchmarks/openmc_xs/` and the large rasters/PBF; keep the `SOURCES.md` pointers.
